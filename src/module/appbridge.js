...
// 设置埋点统计
function apStatistics (params) {
  apwbBridge('eventStatistics', params)
}
// 配置返回键
function nativeBack (params) {
  apwbBridge('goBack', params)
}
// 设置分类按钮
function apSetRightBtn (params) {
  apwbBridge('setRightTitle', params)
}
// 调用本地,拨打电话
function apTel (telNum) {
  apwbBridge('tel', { 'telNumber': telNum })
}

// 高德地图调用
// {“startLng”:xxx,"startLat":xxx,"endLng":xxx,"endLat":xxx,"destination":xxx}
function apMap (map) {
  apwbBridge('setMapNavi', map)
}
export {
  wbBridgeSetup,
  apwbBridge,
